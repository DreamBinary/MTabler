...
from preprocess.tatr import postprocess
...
def objects_to_structures(objects, tokens, class_thresholds):
    ...
    return table_structures, len(rows), len(columns)
...
class TableExtractionPipeline(object):
    def __init__(self,
        ...
        self.str_model.load_state_dict(
            torch.load(str_model_path, map_location=torch.device(str_device))
        )
        self.str_model.to(str_device)
        self.str_model.eval()

    def __call__(self, img, img_file, tokens=None):
        # out_formats = {}
        ...
        tables_structure, rows, cols = objects_to_structures(objects, tokens, self.str_class_thresholds)
...
def main():
    ...
    for count, img_file in enumerate(img_files):
        ...
        if args.mode == 'recognize':
            extracted_table = pipe.recognize(img, tokens, out_cells=args.csv,
