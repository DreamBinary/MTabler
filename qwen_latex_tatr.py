...
<|im_start|>assistant"""
    q_prefix = "Based on the image table, latex table , caption or html structure, "
    ...
    for i in range(0, length, batch_size):
        ...
        outputs = llm.generate(latex_inputs, sampling_params=latex_sp, use_tqdm=False)
        latex = [output.outputs[0].text for output in outputs]
        for l, img, d, o in zip(latex, imgs, td, ocr):
            # shape_list.append(count_rows_cols(l))
            html, rows, cols = o
            shape_list.append((rows, cols))
            q0 = f'This is a table image. The latex code for the table is as follows:\n{l}\n. The caption is: "{d["caption"]}". The HTML structure is as follows:\n{html}\n'
...
"""
        ...
        outputs = llm.generate(inputs, sampling_params=qa_sp, use_tqdm=False)
...
def ocr():
    ...
    for d in data:
        ...
        ocr_data.append(engine(img))
    # print("OCR done")


p = multiprocessing.Process(target=ocr)
p.start()
process()
postprocess()
