...
OCR_BASE_DIR = "/bohr/ocrr-zlwd/v3/OCRCache"
det_model_dir = os.path.join(OCR_BASE_DIR, "whl", "det", "en", "en_PP-OCRv3_det_infer")
rec_model_dir = os.path.join(OCR_BASE_DIR, "whl", "rec", "en", "en_PP-OCRv4_rec_infer")
table_model_dir = os.path.join(OCR_BASE_DIR, "whl", "table", "ch_ppstructure_openatom_SLANetv2_infer")
# params.layout_model_dir = os.path.join(BASE_DIR, "whl", "layout")
rec_char_dict_path = os.path.join(OCR_BASE_DIR, "dict", "en_dict.txt")
table_char_dict_path = os.path.join(OCR_BASE_DIR, "dict", "table_structure_dict.txt")
...
def get_subject(s, ans):
    try:
        label = re.search(r'[A-Za-z]', ans).group(0).upper()
        subject = f"{label}) {sub_list[l2i[label]]}"
        s["subject"] = label
        return subject
    except:
        logger.info("No subject found---------------------------------------->>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>")
        s["subject"] = "fffffffffff"
        return "C) Computer Science"
...
#         params.table_model_dir = os.path.join(OCR_BASE_DIR, "whl", "table", "ch_ppstructure_openatom_SLANetv2_infer")
#         # params.layout_model_dir = os.path.join(BASE_DIR, "whl", "layout")
#
